...
MODULES["automaps"].MapTier0Sliders = [9,9,9,'Mountain'];   //Zone 72+ (9/9/9 Metal)
MODULES["automaps"].MapTier1Sliders = [9,9,4,'Depths']; //Zone 47-72 (9/9/4 Gems)
...
var lastshouldDoMaps = false
var progressDatabase = {};
var enoughHealth2enoughDamage2 = [];
...
function autoMap() {
    ...
    if (!game.global.mapsUnlocked || !(baseDamage > 0)) {   //if we have no damage, why bother running anything? (this fixes weird bugs)
        enoughDamage = true; enoughHealth = true; shouldFarm = false; shouldDoMaps = false;
...
//SAVE CURRENT PROGRESS:
    progressDatabase.stuck = false;
    progressDatabase.lastClearedCell = game.global.lastClearedCell;
    progressDatabase.world = game.global.world;
    progressDatabase.HDratio = HDratio;
    //progressDatabase.deaths = 0;
    progressDatabase.breedtimer = getPageSetting('GeneticistTimer');
    progressDatabase.aliveTime = 0;
    progressDatabase.alivePCT = 0;
...
//BEGIN AUTOMAPS DECISIONS:
    ...
    if (ourBaseDamage > 0){
        enoughHealth2enoughDamage2 = autoStanceCheck(false,true,false);    //maxormin,disableFlucts,enemyCrit
        if (enoughHealth2enoughDamage2[0] != null) {
            gotStuck = !enoughHealth2enoughDamage2[0];
            if (gotStuck)
                debug("Got Stuck.... on Z: " + game.global.world + " ,Cell: " + (game.global.lastClearedCell + 2));            
        }
        shouldDoMaps = !enoughDamage || shouldFarm || scryerStuck || gotStuck;
    }
...
//            if (!game.portal.Overkill.locked && game.stats.cellsOverkilled.value)
    ...
    // var un1 = ['The Wall',15,(game.upgrades.Bounty.allowed == 0 && !game.talents.bounty.purchased)];
    // var un2 = ['Dimension of Anger',20,(document.getElementById("portalBtn").style.display == "none" && !game.talents.portal.purchased)];
    // var un3 = ['The Prison',80,(game.global.challengeActive == "Electricity" || game.global.challengeActive == "Mapocalypse")];
    // var un4 = ['The Block',11,(!game.upgrades.Shieldblock.allowed && (game.global.challengeActive == "Scientist" || game.global.challengeActive == "Trimp" || getPageSetting('BuyShieldblock')))];
    // var un5 = ['Trimple Of Doom',33,(game.global.challengeActive == "Meditate")];
    // var un6 = ['Bionic Wonderland',125,(game.global.challengeActive == "Crushed")];
    // var uniqueMaps = {un1,un2,un3,un4,un5,un6};
    ...
    for (var map in game.global.mapsOwnedArray) {
        ...
        if (theMap.noRecycle && getPageSetting('RunUniqueMaps')) {
            if ((theMap.name == 'The Block' && !game.upgrades.Shieldblock.allowed && (game.global.challengeActive == "Scientist" || game.global.challengeActive == "Trimp" || getPageSetting('BuyShieldblock')))
                ||
                (theMap.name == 'The Wall' && game.upgrades.Bounty.allowed == 0 && !game.talents.bounty.purchased)
                ||
                (theMap.name == 'Dimension of Anger' && document.getElementById("portalBtn").style.display == "none" && !game.talents.portal.purchased)
                ||
                (theMap.name == 'Trimple Of Doom' && (game.global.challengeActive == "Meditate" || game.global.challengeActive == "Trapper"))
                ||
                (theMap.name == 'The Prison' && (game.global.challengeActive == "Electricity" || game.global.challengeActive == "Mapocalypse"))
                ||
                (theMap.name == 'Bionic Wonderland' && game.global.challengeActive == "Crushed" )) {
...
//MAPS CREATION pt2:        
        ...
        if (selectedMap == "world") {
            ...
            if (game.global.world >= customVars.MapTierZone[0]) {
                //Zone 72+ (9/9/9 Metal):
                biomeAdvMapsSelect.value = useGardens ? "Plentiful" : customVars.MapTier0Sliders[3];  //metal is the current meta
                decrement = [];
            } else if (game.global.world >= customVars.MapTierZone[1]) {
                //Zone 47-72 (9/9/4 Gems):
                biomeAdvMapsSelect.value = useGardens ? "Plentiful" : customVars.MapTier1Sliders[3];  //gems is the current meta
                decrement = ['loot'];
            } else if (game.global.world >= customVars.MapTierZone[2]) {
                //Zone 16-47 (9/9/0 Random):
                biomeAdvMapsSelect.value = useGardens ? "Plentiful" : customVars.MapTier2Sliders[3];
                decrement = ['loot'];
            } else {
                //Zone 6-16 (9/0/0 Random):
                biomeAdvMapsSelect.value = useGardens ? "Plentiful" : customVars.MapTier3Sliders[3];
                decrement = ['diff','loot'];
            }
...
function updateAutoMapsStatus() {
    ...
    else if (gotStuck) status.innerHTML = 'Got Stuck! Do maps...';
