...
function autoStanceCheck(maxormin,disableFlucts,enemyCrit) {
    if (game.global.gridArray.length === 0) return [null,null];
//if (game.global.preMapsActive) return [null,null];
    ...
    //COMMON:
    var corrupt = game.global.world >= mutations.Corruption.start();
    var enemy = getCurrentEnemyWorld();
    if (typeof enemy === 'undefined') return [null,null];
    var enemyHealth = enemy.health;
    var enemyDamage = calcBadGuyDmg(enemy,null,true,maxormin,disableFlucts);   //(enemy,attack,daily,maxormin,disableFlucts)
    //crits
    var critMulti = 1;
    var isCrushed = (game.global.challengeActive == "Crushed") && game.global.soldierHealth > game.global.soldierCurrentBlock;        
    var isCritVoidMap = game.global.voidBuff == 'getCrit' || (enemy.corrupted == 'corruptCrit');        
    ...
    var survive = ourHealth > enemyDamage;
